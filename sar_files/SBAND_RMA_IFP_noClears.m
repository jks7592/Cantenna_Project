...
%tic %this starts a timer which will conlude at the end of the program
load Phase1;
...
if do_all_plots == 1
    ...
    print(gcf, '-djpeg', 'phase_before_along_track_fft.jpg');
...
sprintf('Along track FFT section complete at %f seconds', toc) % Added so that I can check progress
...
if do_all_plots == 1
    ...
    print(gcf, '-djpeg', 'mag_after_along_track_fft.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
...
if do_all_plots == 1
    ...
    cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'radians','fontsize',13); 
    print(gcf, '-djpeg', 'phase_after_along_track_fft.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
...
if do_all_plots == 1
    ...
    cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'dB','fontsize',13); 
    print(gcf, '-djpeg', 'mag_after_2D_fft.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
...
%**********************************************************************
%matched filter
%Jason Notes: I cant find where 'Rs' gets defined but it doesn't break the
%code, I don't know what's going on here. 
...
%end
%smf = exp(1j*phi_mf); %%%%%%%%%%%%
smf=1; % Jason Note: added this in order to keep naming convention same
...
if do_all_plots == 1
    ...
    cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'radians','fontsize',13); 
    print(gcf, '-djpeg', 'phase_after_matched_filter.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
...
if do_all_plots == 1
    ...
    cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'dB','fontsize',13); 
    print(gcf, '-djpeg', 'mag_after_downrange_fft_of_matched_filtered_data.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
...
sprintf('Matched Filter section complete at %f seconds', toc) % Added so that I can check progress
%**********************************************************************
...
S_st(isnan(S_st)) = 1E-30; %set all Nan values to 0
%clear S_mf ii Ky;
...
if do_all_plots == 1
    ...
    cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'radians','fontsize',13); 
    print(gcf, '-djpeg', 'phase_after_stolt_interpolation.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
...
sprintf('Stolt Interpolation section complete at %f seconds', toc)% Added so that I can check progress
%-----------------------------------------------------
%#####Warning, this section found to not be used later  in code#####
...
sprintf('Data truncation complete at %f seconds', toc)% Added so that I can check progress

%scale down range columns by range^(3/2), delete to make like dissertation again
clear ii;
...
imagesc(crossrange, downrange, trunc_image, [max(trunc_image(:))-40, max(trunc_image(:))-0]);  
colormap('default');
title('Final Image');
ylabel('Downrange (ft)');
...
cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'dB','fontsize',13); 
    print(gcf, '-djpeg', 'final_image.jpg');
    if close_as_you_go == 1
        close(figcount);
    end
    figcount = figcount + 1;
    
sprintf('Phase 2 of processing took %f seconds', toc) %toc displays final timer value
